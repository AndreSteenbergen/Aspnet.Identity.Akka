...
{
    public class TokenAddedEventFormatter : IMessagePackFormatter<TokenAdded>
    {
        public TokenAdded Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize)
        {
            ...
            return new TokenAdded(s1, s2, s3);
        }

        public int Serialize(ref byte[] bytes, int offset, TokenAdded value, IFormatterResolver formatterResolver)
        {
